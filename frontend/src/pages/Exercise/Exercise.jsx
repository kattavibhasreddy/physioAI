...
// MediaPipe Pose landmark indices that map to the original MoveNet 17 keypoints.
// MediaPipe Pose outputs 33 landmarks; the first 17 match MoveNet's keypoint order exactly:
...
//
// We remap them to MoveNet's 17-keypoint order so the backend data format stays identical.
const MEDIAPIPE_TO_MOVENET_INDICES = [
  0,   // nose
  2,   // left_eye  (MediaPipe "left_eye" is index 2)
  5,   // right_eye (MediaPipe "right_eye" is index 5)
  7,   // left_ear
  8,   // right_ear
  11,  // left_shoulder
  12,  // right_shoulder
...
class Exercise extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      keypoint_dict: { // Dictionary to map keypoints to their indices (MoveNet order)
        'nose': 0,
        'left_eye': 1,
        'right_eye': 2,
        'left_ear': 3,
  ...
  async componentDidMount() {
    ...
    try {
      ...
      console.log("MediaPipe Pose setup complete")
  ...
  poseDetectionFrame(canvasContext) {
    ...
    const findPoseDetectionFrame = async () => {
      ...
      // Detect poses using MediaPipe PoseLandmarker
      const result = this.poseLandmarker.detectForVideo(video, performance.now())

      // Convert MediaPipe landmarks to MoveNet-compatible format for the renderer
      let normalizedKeypoints = null
      if (result.landmarks && result.landmarks.length > 0) {
        const landmarks = result.landmarks[0] // First detected pose

        // Build poses array compatible with the renderer (pixel coordinates for drawing)
        const keypointsForRenderer = MEDIAPIPE_TO_MOVENET_INDICES.map((mpIdx) => {
        ...
        poses = [{ keypoints: keypointsForRenderer }]

        // Normalized keypoints for DataFrame (values in 0-1 range, matching CSV format)
        normalizedKeypoints = MEDIAPIPE_TO_MOVENET_INDICES.map((mpIdx) => {
          const lm = landmarks[mpIdx]
          return {
            x: lm.x,
            y: lm.y,
